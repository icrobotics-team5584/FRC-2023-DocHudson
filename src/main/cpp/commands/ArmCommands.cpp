...
frc2::CommandPtr ArmToSafePosition() {
  return Either(ArmSafePos().Until([] {
    return SubArm::GetInstance().GetEndEffectorPosition().Y() > 60_cm;
  }),
                None(), [] {
                  return SubArm::GetInstance().GetEndEffectorPosition().Y() <
                         60_cm;
                });
}
...
frc2::CommandPtr ArmToHighCone() {
  return ArmToSafePosition().AndThen(ArmToPos(138_cm, 114_cm));
}  // (x_cm, y_cm)
frc2::CommandPtr ArmToScoredHighCone() {
  return ArmToSafePosition().AndThen(ArmToPos(150_cm, 85_cm));
}
frc2::CommandPtr ArmToMidCone() {
  return ArmToSafePosition().AndThen(ArmToPos(115_cm, 85_cm));
}  // gtg

frc2::CommandPtr ArmToHighCube() {
  return ArmToSafePosition().AndThen(ArmToPos(146.8_cm, 98_cm));
}
frc2::CommandPtr ArmToMidCube() {
  return ArmToSafePosition().AndThen(ArmToPos(104.4_cm, 52.1_cm));
}

frc2::CommandPtr ArmToLowCubeOrCone() {
  return ArmToPos(45_cm, 15_cm);
}
frc2::CommandPtr ArmToLoadingStation() {
  return ArmToPos(0.703_m, 0.822_m);
}
frc2::CommandPtr ArmToDefaultLocation() {
  return ArmToPos(47_cm, 5_cm);
}  // gtg

frc2::CommandPtr ArmPickUp() {
  return RunOnce(
             []() {
               std::cout << "before SubArm";
               SubArm::GetInstance().DriveTo(0.2020_tr, -0.389_tr);
               std::cout << "After DriveTo";
             })
      .AndThen(
          WaitUntil([]() { return SubArm::GetInstance().CheckPosition(); }));
}

frc2::CommandPtr CubeConeSwitch() {
  return RunOnce([] {
    if (RobotContainer::isConeMode) {
      RobotContainer::isConeMode = false;
    } else {
      RobotContainer::isConeMode = true;
    }
  });
}

frc2::CommandPtr ArmToHigh() {
  return Either(ArmToHighCone(), ArmToHighCube(),
                [] { return RobotContainer::isConeMode; });
}

frc2::CommandPtr ArmToMid() {
  return Either(ArmToMidCone(), ArmToMidCube(),
                [] { return RobotContainer::isConeMode; });
}
...
frc2::CommandPtr CoastModeOverride() {
  return StartEnd(
             [] {
               ...
               SubArm::GetInstance().SetIdleMode(
